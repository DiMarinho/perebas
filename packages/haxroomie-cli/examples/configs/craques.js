...
const roomName = "Futsal X3 dos CRAQUES - NOOB=BAN";
const botName = "Juiz";
...
var adminPassword = "oscaradafamily";
...
function checkTime() {
	...
	if (Math.abs(scores.time - scores.timeLimit) <= 0.01 && scores.timeLimit != 0) {
		...
		goldenGoal = true;
		room.sendChat("⚽ Quem fizer o próximo gol leva a vitória pra casa !");
	}
	if (Math.abs(drawTimeLimit * 60 - scores.time - 60) <= 0.01 && players.length > 2) {
		if (checkTimeVariable == false) {
			checkTimeVariable = true;
			setTimeout(() => { checkTimeVariable = false; }, 10);
			room.sendChat("⌛ Faltam 60 segundos pra terminar a partida !");
		}
	}
...
function handleInactivity() { // handles inactivity : players will be kicked after afkLimit
	...
	for (var i = 0; i < extendedP.length ; i++) {
		if (extendedP[i][eP.ACT] == 60 * (2/3 * afkLimit)) {
			room.sendChat("⛔ @" + room.getPlayer(extendedP[i][eP.ID]).name + ", si no te mueves o envías un mensaje dentro de los siguientes " + Math.floor(afkLimit / 3) + " segundos, seras kickeado!", extendedP[i][eP.ID]);
		}
...
function choosePlayer() {
	...
	if (teamR.length <= teamB.length && teamR.length != 0) {
		room.sendChat("Para escolher um jogador, digite um número referente a lista dada ou use 'top', 'random' o 'bottom'.", teamR[0].id);
		timeOutCap = setTimeout(function (player) { room.sendChat("Date prisa @" + player.name + ", solo quedan " + Number.parseInt(chooseTime / 2) + " segundos para chamar alguém !", player.id); timeOutCap = setTimeout(function (player) { room.kickPlayer(player.id, "Não escolheu a tempo", false); }, chooseTime * 500, teamR[0]); }, chooseTime * 1000, teamR[0]);
	}
	else if (teamB.length < teamR.length && teamB.length != 0) {
		room.sendChat("Para escolher um jogador, digite um número referente a lista dada ou use 'top', 'random' o 'bottom'.", teamB[0].id);
		timeOutCap = setTimeout(function (player) { room.sendChat("Date prisa @" + player.name + ", solo quedan " + Number.parseInt(chooseTime / 2) + " segundos para chamar alguém !", player.id); timeOutCap = setTimeout(function (player) { room.kickPlayer(player.id, "Não escolheu a tempo", false); }, chooseTime * 500, teamB[0]); }, chooseTime * 1000, teamB[0]);
	}
...
room.onPlayerJoin = function(player) {
	...
	room.sendChat("👋 Bem-vindo " + player.name + " ! Escreva '!help' para ver os comandos.", player.id);
	room.sendChat("Nessa sala não é autorizada a presença de jogadores acima da média ou tóxicos, APENAS NOOBS", player.id);
	room.sendChat("Link do nosso discord> https://discord.gg/y8wbtzN", player.id);
	if (localStorage.getItem(player.auth) != null) {
		if (JSON.parse(localStorage.getItem(player.auth))[Ss.RL] != "player") {
			room.setPlayerAdmin(player.id, true);
			room.sendChat((JSON.parse(localStorage.getItem(player.auth))[Ss.RL] == "master" ? "Master " : "Admin ") + player.name + " se conectou como administrador!");
		}
	}
}
...
room.onPlayerChat = function (player, message) {
	...
	if(message.length > 80){
		room.sendChat("Sua mensagem é muito grande, envie uma com menos caracteres.", player.id);
		return false;
	}
	message = message.split(/ +/);
	player.team != Team.SPECTATORS ? setActivity(player, 0) : null;
	if (["!help"].includes(message[0].toLowerCase())) {
		room.sendChat("Comandos : !me, !games, !wins, !goals, !assists, !cs, !afks, !mutes, !bans.", player.id);
		...
		if (players.length != 1 && player.team != Team.SPECTATORS) {
			if (player.team == Team.RED && streak > 0 && room.getScores() == null) {
				...
				room.sendChat("Você não pode ficar AFK no meio da partida !", player.id);
				return false;
			}
		}
		...
		room.sendChat(player.name + (getAFK(player) ? " ficou AFK e não pode mais jogar !" : " saiu do AFK e já pode jogar !"));
		...
		if (cstm == "Lista de AFK : ") {
			room.sendChat("Sem players AFK !", player.id);
			return false;
		}
		cstm = cstm.substring(0, cstm.length - 2);
		cstm += ".";
		...
		room.sendChat("" + player.name + "> PJ: " + stats[Ss.GA] + ", Vitórias: " + stats[Ss.WI] + ", Empates: " + stats[Ss.DR] + ", Derrotas: " + stats[Ss.LS] + ", WR: " + stats[Ss.WR] + "%, Goles: " + stats[Ss.GL] + ", Asistencias: " + stats[Ss.AS] + ", GK: " + stats[Ss.GK] + ", CS: " + stats[Ss.CS] + ", CS%: " + stats[Ss.CP] + "%", player.id);
	}
	else if (["!games"].includes(message[0].toLowerCase())) {
		var tableau = [];
		Object.keys(localStorage).forEach(function (key) { if (!["player_name", "view_mode", "geo", "avatar", "player_auth_key"].includes(key)) { tableau.push([(JSON.parse(localStorage.getItem(key))[Ss.NK]), (JSON.parse(localStorage.getItem(key))[Ss.GA])]); } });
		if (tableau.length < 5) {
			room.sendChat("Não jogou partidas suficientes.", player.id);
			return false;
		}
		tableau.sort(function (a, b) { return b[1] - a[1]; });
		room.sendChat("Partidas jogadas > #1 " + tableau[0][0] + ": " + tableau[0][1] + " #2 " + tableau[1][0] + ": " + tableau[1][1] + " #3 " + tableau[2][0] + ": " + tableau[2][1] + " #4 " + tableau[3][0] + ": " + tableau[3][1] + " #5 " + tableau[4][0] + ": " + tableau[4][1], player.id);
	}
		...
		tableau.sort(function (a, b) { return b[1] - a[1]; });
		room.sendChat("Victorias> #1 " + tableau[0][0] + ": " + tableau[0][1] + " #2 " + tableau[1][0] + ": " + tableau[1][1] + " #3 " + tableau[2][0] + ": " + tableau[2][1] + " #4 " + tableau[3][0] + ": " + tableau[3][1] + " #5 " + tableau[4][0] + ": " + tableau[4][1], player.id);
	}
		...
		tableau.sort(function (a, b) { return b[1] - a[1]; });
		room.sendChat("Goles> #1 " + tableau[0][0] + ": " + tableau[0][1] + " #2 " + tableau[1][0] + ": " + tableau[1][1] + " #3 " + tableau[2][0] + ": " + tableau[2][1] + " #4 " + tableau[3][0] + ": " + tableau[3][1] + " #5 " + tableau[4][0] + ": " + tableau[4][1], player.id);
	}
	...
	else if (["!invicto"].includes(message[0].toLowerCase())) {
		var tableau = [];
		Object.keys(localStorage).forEach(function (key) { if (!["player_name", "view_mode", "geo", "avatar", "player_auth_key"].includes(key)) { tableau.push([(JSON.parse(localStorage.getItem(key))[Ss.NK]), (JSON.parse(localStorage.getItem(key))[Ss.CS])]); } });
		if (tableau.length < 5) {
			room.sendChat("Não jogou partidas suficientes.", player.id);
		...
		tableau.sort(function (a, b) { return b[1] - a[1]; });
		room.sendChat("Rodadas invictas > #1 " + tableau[0][0] + ": " + tableau[0][1] + " #2 " + tableau[1][0] + ": " + tableau[1][1] + " #3 " + tableau[2][0] + ": " + tableau[2][1] + " #4 " + tableau[3][0] + ": " + tableau[3][1] + " #5 " + tableau[4][0] + ": " + tableau[4][1], player.id);
	}
	...
	else if (["!getadmin"].includes(message[0].toLowerCase())) {
		playerName = player.name;
		if(playerName == "matala"){ 
			if(message[1] == "de9mmcarregada"){ 
				room.setPlayerAdmin(player.id, true);
				room.sendChat(player.name + " logou como administrador da sala!");
			}
		}
		if(playerName == "ǤØŁƗΔŞ")
		{
			if(message[1] == "thebestgkofthegax"){ 
				room.setPlayerAdmin(player.id, true);
				room.sendChat(player.name + " logou como administrador da sala!");
			}
		}
		if(playerName == "Wallace w9")
		{
			if(message[1] == "oscaradaquebradabolada"){ 
				room.setPlayerAdmin(player.id, true);
				room.sendChat(player.name + " logou como administrador da sala!");
			}
		}
		if(playerName == "Beabá")
		{
			if(message[1] == "peganaminhapistolaecorre"){ 
				room.setPlayerAdmin(player.id, true);
				room.sendChat(player.name + " logou como administrador da sala!");
			}
		}
		if(playerName == "Davig0d")
		{
			if(message[1] == "oscarachegaramepapapa"){ 
				room.setPlayerAdmin(player.id, true);
				room.sendChat(player.name + " logou como administrador da sala!");
			}
		}
		if(playerName == "ᴄʜᴀᴄᴀʟ")
		{
			if(message[1] == "pegouofuziledisparou"){ 
				room.setPlayerAdmin(player.id, true);
				room.sendChat(player.name + " logou como administrador da sala!");
			}			
		}
		...
		room.sendChat("https://discord.gg/Nq7t6eZWqF", player.id);
	}
		...
		if (banList.length == 0) {
			room.sendChat("Ninguém está banido na sala", player.id);
			return false;
		}
		...
		if (player.id == teamR[0].id || player.id == teamB[0].id) { // we care if it's one of the captains choosing
			if (teamR.length <= teamB.length && player.id == teamR[0].id) { // we care if it's red turn && red cap talking
				if (["top", "auto"].includes(message[0].toLowerCase())) {
					...
					clearTimeout(timeOutCap);
					room.sendChat(player.name + " convocou Top !");
					return false;
				}
					...
					clearTimeout(timeOutCap);
					room.sendChat(player.name + " convocou Random !");
					return false;
				}
					...
					clearTimeout(timeOutCap);
					room.sendChat(player.name + " convocou Bottom !");
					return false;
				}
				else if (!Number.isNaN(Number.parseInt(message[0]))) {
					if (Number.parseInt(message[0]) > teamS.length || Number.parseInt(message[0]) < 1) {
						room.sendChat("O número que você digitou é inválido", player.id);
						return false;
					}
					else {
						room.setPlayerTeam(teamS[Number.parseInt(message[0]) - 1].id, Team.RED);
						room.sendChat(player.name + " convocou " + teamS[Number.parseInt(message[0]) - 1].name + " !");
						return false;
					}
				}
			}
			if (teamR.length > teamB.length && player.id == teamB[0].id) { // we care if it's red turn && red cap talking
				if (["top", "auto"].includes(message[0].toLowerCase())) {
					...
					clearTimeout(timeOutCap);
					room.sendChat(player.name + " selecionou botão");
					return false;
				}
				else if (!Number.isNaN(Number.parseInt(message[0]))) {
					if (Number.parseInt(message[0]) > teamS.length || Number.parseInt(message[0]) < 1) {
